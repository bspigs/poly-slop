...
def run(
    ...
    if demo:
        ...
        try:
            markets = [fetch_current_btc_15m_market()]
    ...
    if not ranked or max_research <= 0:
        return
    ...
    for market in research_queue[:max_research]:
        ...
        decision = (
            decide_baseline_trade(market, estimate, SETTINGS.starting_bankroll)
            if baseline_paper
            else decide_trade(market, estimate, SETTINGS.starting_bankroll)
        )
        label = _side_label(decision.side, decision.positive_label, decision.negative_label)
        console.print(
            f"Market {market.positive_label} {market.yes_price:.1%} | "
            f"fair {market.positive_label} {estimate.fair_yes_probability:.1%} | "
            f"decision [bold]{label}[/bold] | edge {decision.edge:.1%} | "
        ...
        if execute_paper or baseline_paper:
            ...
            if position:
                console.print(f"[green]PAPER TRADE EXECUTED: {label} ${position.stake:,.2f}[/green]")


def cli() -> None:
    parser = argparse.ArgumentParser(description="Polymarket research and paper-trading agent")
    parser.add_argument("--research", type=int, default=5)
    ...
    parser.add_argument("--btc-15m", action="store_true")
    parser.add_argument(
        "--btc-scalp",
        action="store_true",
    ...
    if args.btc_scalp:
        try:
            scalp_one_current_window(args.provider, console=console)
        except Exception as exc:
            console.print(f"[red]BTC scalp failed:[/red] {exc}")
        return
